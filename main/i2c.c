...
void i2c_init(void)
{
	...
	{
		module_data[module].buses = 1; // FIXME

		for(bus = i2c_bus_first; bus < i2c_bus_size; bus++)
			module_data[module].bus[bus] = (bus_t *)0;

		util_abort_on_esp_err("i2c_new_master_bus", i2c_new_master_bus(&module_config[module], &module_data[module].handle));
	}
...
i2c_slave_t i2c_register_slave(const char *name, i2c_module_t module, i2c_bus_t bus, unsigned int address)
{
	...
	{
		bus_ptr = module_ptr->bus[bus] = (bus_t *)util_memory_alloc_spiram(sizeof(*bus_ptr));
		bus_ptr->id = bus;
...
bool i2c_unregister_slave(i2c_slave_t *slave)
{
	...
	if(bus->slaves->address == (**_slave).address)
	{
		...
		if(!(bus->slaves = (**_slave).next))
		{
			module->bus[(**_slave).bus] = (bus_t *)0;
			free(bus);
		}
...
bool i2c_probe_slave(i2c_module_t module, i2c_bus_t bus, unsigned int address)
{
	...
	rv = i2c_master_probe(module_data[module].handle, address, 500);
...
i2c_slave_t i2c_find_first_slave(i2c_module_t module, i2c_bus_t bus, unsigned int address)
{
	...
	{
		moduleptr = &module_data[module_index];

		if((module != i2c_module_any) && (module_index != module))
			continue;

		for(bus_index = i2c_bus_first; bus_index < i2c_bus_size; bus_index++)
		{
			busptr = moduleptr->bus[bus_index];
			...
			if(busptr)
			{
				for(slaveptr = busptr->slaves; slaveptr; slaveptr = slaveptr->next)
				{
					...
					if((module != i2c_module_any) && (slaveptr->module != i2c_module_any) && (slaveptr->module != module))
						continue;
...
void command_i2c_info(cli_command_call_t *call)
{
	...
	for(module_index = i2c_module_first; module_index < i2c_module_size; module_index++)
	{
		...
		for(bus_index = i2c_bus_first; bus_index < buses; bus_index++)
		{
			if((bus = module->bus[bus_index]))
			{
				string_format_append(call->result, "\n-  bus [%u/%u]", bus->id, buses - 1);
